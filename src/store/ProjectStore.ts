...
import { FRONTMATTER_KEY, TASK_FRONTMATTER_KEY, parseFrontmatter, projectDescriptionFromBody, stripAutoGeneratedContent } from './YamlParser'
import {
...
export class ProjectStore implements TaskSource {
  ...
  async loadProjectBody(project: Project): Promise<void> {
    ...
    const content = await this.app.vault.cachedRead(file)
    const { frontmatter, body } = parseFrontmatter(content)
    project.description = projectDescriptionFromBody(frontmatter, body)
    this.hydratedBodies.add(project)
  }
